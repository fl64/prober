...
RUN \
  ...
  \
  && curl -L https://github.com/pete911/certinfo/releases/download/v1.0.30/certinfo_1.0.30_linux_amd64.tar.gz | tar -zx -C /usr/local/bin/ certinfo \
  && chmod +x  /usr/local/bin/certinfo \
  \
  && curl -L https://github.com/goss-org/goss/releases/download/v0.4.9/goss-linux-amd64 -o /usr/local/bin/goss \
  && chmod +x  /usr/local/bin/goss \
  \
  && curl -L https://github.com/google/go-containerregistry/releases/download/v0.20.2/go-containerregistry_Linux_x86_64.tar.gz | tar -xz -C /usr/local/bin/ crane \
